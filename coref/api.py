...
def parse_manual_mentions(text):
    ...
    # Extract tokens
    for sentence_idx, sentence in enumerate(sent_tokenize(text)):
        token_count = 0
        next_is_start = 0 # Counts the number of mentions starting at the next token
        for idx, token in enumerate(word_tokenize(sentence)):
            print(sentence_idx, token_count, token)
            if token == '[':
                next_is_start += 1
            elif token == ']':
                doc.tokens[-1].num_mention_ends += 1
            else:
                t = Token([token_count, token], sentence_idx)
